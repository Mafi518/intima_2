...
interface Props {
  value: number; // Значение может быть с плавающей запятой
}

const { value } = defineProps<Props>();
const filledStars = Math.floor(value); // Целая часть для заполненных звезд
const hasHalfStar = value % 1 > 0; // Проверяем есть ли половинчатая звезда
</script>
...
<template>
  <div class="rating">
    ...
    >
      <defs>
        <linearGradient id="halfStar">
          <stop offset="50%" stop-color="#8c5aff" />
    ...
    <svg
      v-for="index in 5 - filledStars - (hasHalfStar ? 1 : 0)"
      :key="'empty-' + index"
      width="14"
      height="13"
      viewBox="0 0 14 13"
